...
G1_1 + (G1_2 / G1_3) + plot_layout(ncol=2,widths=c(2,1))

```



Con cualquiera de los gráficos podemos ver que a lo largo de los años, este comportamiento se mantiene, registrándose una diferencia importante entre los porcentajes de desempleo. Incluso cuando se trata de hombres y mujeres con educación terciaria, las diferencias siguen ahí.
...
```{r, fig.width=10, fig.height=5, echo = FALSE, warning=FALSE, fig.align='center'}
...
```{r, fig.width=10, fig.height=4, echo = FALSE, warning=FALSE, fig.align='center'}
...
G6 <- ggplot(empleadores, aes(Anio,porcentaje, fill = sexo, na.rm = TRUE)) +
...
```{r, fig.width=10, fig.height=4, echo = FALSE, fig.align='center'}
...
G8 <- ggplot(actividades_mujeres, aes(media, variable, fill = variable)) +
  geom_col(position=position_dodge()) +
  labs(title = "Gráfico 8 - Distribución de actividad y género")

G8
```
En el gráfico 8 podemos apreciar que en gran porcentaje las mujeres realizan trabajo parcial, informal o trabajo doméstico lo que implica menos ingresos y nos preguntamos ¿Los trabajos domésticos, le resta tiempo a las mujeres para realizar trabajo remunerado?
...
Incluso una mujer que trabaja (fuera del hogar y de manera paga) en una jornada completa dedica más horas a estas tareas, en otros casos las mujeres trabajan en forma parcial o trabajo informal para dedicarle tiempo a la crianza de sus hijos o al cuidado de un familiar, ya que estas tareas en muchos casos le quitan tiempo para realizar trabajo remunerado.
Así las cosas, las mujeres quedan inmersas en una suerte de círculo vicioso, en el que no solo dedican gran parte de su tiempo a este tipo de tareas no remuneradas, sino que, al hacerlo, acrecientan sus desventajas con respecto al mercado laboral remunerado. Lamentablemente es una tarea que se asocia al rol femenino y a pesar que en los últimos años pareciera que hay más participación en las tareas domésticas por parte de los hombres la brecha sigue siendo importante.
...
Sin lugar a dudas, el análisis realizado permite concluir que aún existe una amplia brecha
laboral de género. El objetivo de igual remuneración por igual trabajo es una promesa
incumplida. La brecha con los hombres es todavía importante, y demuestra que a pesar de tener 
los mismos niveles de formación profesional las mujeres no tienen las mismas oportunidades.
Por último, cabe destacar que la mujer emplea gran parte de su tiempo a las tareas domésticas y 
eso le resta tiempo a un trabajo remunerado que le permita desarrollo económico.
