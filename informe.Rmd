...
desempleo_calificado <- filter(empleo_Arg,variable %in% c("desempleo_educacion_mujeres","desempleo_educacion_hombres")) %>%
            mutate(sexo = ifelse(variable == "desempleo_educacion_mujeres",0,1))

ggplot(desempleo_calificado,aes(Anio,cantidad, fill = variable)) + geom_col() + labs(title = "Desempleo femenino y masculino, con educación terciaria, por año")
```
...
tipo_empleo <- filter(empleo_Arg,variable %in% c("empleo_agricultura_mujeres","empleo_agricultura_hombres","empleo_industria_mujeres","empleo_industria_hombres","empleo_servicios_mujeres","empleo_servicios_hombres","empleo_informal_mujeres","empleo_informal_hombres","autoempleo_mujeres","autoempleo_hombres")) %>%
            mutate(sexo = ifelse(str_detect(variable,"mujeres"),0,1), 
                  tipo = substring(variable,8,str_length(variable)-8)
```

```{r}
var <- c("empleo_agricultura_mujeres","empleo_industria_mujeres")
Tip <- substring(var,8,str_length(var)-8)
