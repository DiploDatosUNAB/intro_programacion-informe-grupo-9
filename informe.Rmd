...
ggplot(desempleo,aes(Anio,porcentaje)) + 
  geom_line(aes(color = sexo),lwd = 2) +
  labs(title = "Gráfico 1 - Desempleo femenino y masculino por año")
```
...
```{r}
ggplot(Jefes_vs_desempleocal, aes(porcentaje, variable, fill = sexo, na.rm = TRUE)) +
  geom_col(position=position_dodge()) + 
  scale_fill_manual(values=c("#DC143C", "#2E8B57")) +
...
boxplot(porcentaje ~ variable, data = Jefes_vs_desempleocal, col = "#F0E68C")

boxplot(Jefes_vs_desempleocal$porcentaje ~ Jefes_vs_desempleocal$variable, 
        col = rainbow(ncol(Jefes_vs_desempleocal)), 
        outbg = "black") +
  labs(title = "Gráfico XXXXXX - Empleadores vs desempleo calificado")
