...
Desempleo es una tabla de 4 variables y 70 observaciones. 
...
empleadores <- dplyr::filter(empleo_Arg, variable %in% c("empleadoras_mujeres","empleadores_hombres")) %>% 
 ...
 # # Busco la distribución de la ocupación de las mujeres
  ...
  # y ahora se lo extraigo y renombro la columna a fuerza laboral
  ...
  EmpleoAR1 <- gt(head(empleo_genero1,5)) |> 
      tab_header(title = md("**Distribución del empleo en Argentina**"),
                 subtitle = "Años 1991 a 2018") |> 
      cols_label(Anio = "Año", variable = "variable", sexo = "Sexo", 
...
EmpleoAR <- gt(head(empleo_Arg_ancho,5)) |> 
      tab_header(title = md("**Distribución del empleo en Argentina**"),
      ...
      cols_label(Anio = "Año", empleadoras_mujeres = "Empleadoras", empleadores_hombres = "Empleadores", 
                 ...
                 desempleo_educacion_mujeres = "Desempleo calificado M", desempleo_educacion_hombres = "Desempleo calificado H", 
                 desempleo_mujeres = "Desempleo M", desempleo_hombres = "Desempleo H", 
                 trabajo_domestico_no_remunerado_mujeres = "Trab Domestico M", trabajo_domestico_no_remunerado_hombres = "Trab doméstico H") |>
    cols_align(align = "center", columns = everything()) |> 
    tab_source_note(md("*Fuente: Banco Mundial sobre empleo/desempleo y género para Latinoamérica y Caribe (1970 a 2018)*")) 
...
G1_1 <- ggplot(desempleo,aes(Anio,porcentaje, fill = sexo)) + geom_col(position=position_dodge()) + 
  ...
  labs(title = "Gráfico 1 - Desempleo por género y año", x = "Años", y = "Distribución (%)") +
...
Esta tabla contiene información sobre el tipo de actividad realizada en el empleo. También está por género y año. Una observación a tener en cuenta es que, si bien se releva el empleo parcial para las mujeres, no se relevó para los hombres. Por lo cual esta variable no puede ser comparada.
```{r warning=FALSE, fig.align='center', fig.alt="El gráfico 2 es un gráfico de barras que representa el tipo de actividades(Agrigultura, Autoempleo, Industria, Trabajo-Informal y servicios) en el eje x y la distribucion en porcentaje en el eje y del 0 al 75%. Se compara según sexo y lo que podemos apreciar es que en el sector de servicios las mujeres superan el 80% y en la industria, donde la brecha está muy marcada, no supera el 12%."}
...
G2 <- ggplot(tipo_empleo,aes(tipo,porcentaje, fill = sexo)) + 
  ...
  scale_fill_manual(values=c("#213A40", "#D9325E")) +
  theme_classic()+
  labs(title = "Gráfico 2 - Distribución del empleo por tipo de actividad y género", x = "Tipo de Actividad", y = "Distribución (%)")
...
```{r, fig.width=10, fig.height=5, fig.align='center', warning=FALSE, fig.alt="Desempleo calificado"}
...
G3 <- ggplot(desempleo_calificado,aes(Anio,porcentaje, fill = sexo, na.rm = TRUE)) +  
  ...
  geom_col(position=position_dodge()) + 
  scale_fill_manual(values=c("#213A40", "#D9325E")) +
  labs(title = "Gráfico 3 - Desempleo femenino y masculino, con educación terciaria, por año", x = "Años", y = "Distribución (%)") +
...
```{r,fig.width=10, fig.height=5, fig.align='center', warning=FALSE, fig.alt="El gráfico 4 es un gráfico de barras que representa Empleadores por genero y año (años 1990 al 2018) en el eje x y la distribucion en porcentaje en el eje y del 0 al 6%. Se compara según sexo y lo que podemos apreciar es que en el sector de servicios las mujeres superan el 80% y en la industria, donde la brecha está muy marcada, no supera el 12%."}
...
G6 <- boxplot(Jefes_vs_desempleocal$porcentaje ~ Jefes_vs_desempleocal$variable,
        horizontal = TRUE,
        col = c("#3E95AB","#D9325E", "#3E2CAD", "#213A40"), 
        outbg = "black", 
        outpch = 23,
        ylab = "",
        xlab = "Distribución (%)",
...
legend(8, 4, c("Desempleo Calificado Hombres", "Desempleo Calificado Mujeres", "Jefas", "Jefes"),
       fill = c("#3E95AB","#D9325E", "#3E2CAD", "#213A40"))
