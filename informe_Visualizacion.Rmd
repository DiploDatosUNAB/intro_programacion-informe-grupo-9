...
```{r fig.height=5, fig.width=8, message=TRUE, warning=FALSE, fig.alt="Analisis comparativo de dos gráficos respecto a la desocupación en argentina, donde se puede observar la diferencia entre los generos con un promedio del 12.22% para las mujeres y del 9.91% para los hombres."}
G1_1 <- ggplot(desempleo,aes(Anio,porcentaje, fill = sexo)) + geom_col(position=position_dodge()) + 
  ...
  labs(title = "Gráfico 1 - Desempleo por género y año", x = "Año", y = "Porcentaje") +
...
G3 <- ggplot(desempleo_calificado,aes(Anio,porcentaje, fill = sexo, na.rm = TRUE)) +  
  ...
  geom_col(position=position_dodge()) + 
  scale_fill_manual(values=c("#213A40", "#D9325E")) +
  labs(title = "Gráfico 3 - Desempleo femenino y masculino, con educación terciaria, por año", x = "Año", y = "Distribución (%)") +
...
G5 <-empleadores %>% 
  ...
  scale_color_manual(labels= c("Hombres","Mujeres"), values = c("#213A40", "#D9325E"))+
  labs(title = "Gráfico 5 - Empleadores por género y año", x = "Años", y = "Porcentaje %", color = "Sexo")+
  scale_x_continuous(limits=c(1990,2018),breaks = c(1990,1995,2000,2005,2009,2013,2018)) +
  theme_classic()+
  theme(axis.text.x = element_text(size = 9),  
        axis.text.y = element_text(size = 9),
